...
export async function POST(req: Request) {
  ...
  if (!signature) {
    console.error('[STRIPE_WEBHOOK] No signature');
...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session) {
  ...
  const plan = session.metadata?.plan as 'PRO' | 'TEAM_ADMIN';
  ...
  console.log(`[STRIPE_WEBHOOK] ✅ User ${userId} upgraded to ${plan}`);
}
...
async function handleSubscriptionUpdated(subscription: Stripe.Subscription) {
  ...
  console.log(`[STRIPE_WEBHOOK] ✅ Subscription updated for user ${user.id}`);
}
...
async function handleSubscriptionDeleted(subscription: Stripe.Subscription) {
  ...
  console.log(`[STRIPE_WEBHOOK] ✅ Subscription deleted, user ${user.id} downgraded to FREE`);
}
...
async function handlePaymentFailed(invoice: Stripe.Invoice) {
  ...
  console.log(`[STRIPE_WEBHOOK] ⚠️ Payment failed for user ${user.id}`);
